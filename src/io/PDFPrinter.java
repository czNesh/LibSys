...
public class PDFPrinter extends PdfPageEventHelper {
    ...
    public void printNotification(Notification notification) {
        try {
            Document document = new Document(PageSize.A4, 10, 10, 100, 10);
            Borrow borrow = notification.getBorrow();
            PdfWriter.getInstance(document, new FileOutputStream(Configuration.getInstance().getWorkspace() + "\\PDF_NOTITFICATION_" + borrow.getBorrowCode() + ".pdf"));
            ...
            // TYP OZNAMENI
            t.addCell(formatOutput("Typ oznámení", 11, true));
            switch (notification.getType()) {
                case NOT_RETURNED:
                    t.addCell(formatOutput("Nevrácená kniha", 10, false));
                    break;
                case LONG_TIME_BORROW:
                    t.addCell(formatOutput("Dlouhodobě vypůjčená kniha", 10, false));
                    break;
            }
            ...
            t2.getDefaultCell().setHorizontalAlignment(Element.ALIGN_CENTER);
            t2.getDefaultCell().setVerticalAlignment(Element.ALIGN_MIDDLE);
            t2.getDefaultCell().setColspan(2);
            t2.addCell(formatOutput("Kódy výpůjčky", 12, true));
            t2.getDefaultCell().setColspan(1);
            java.awt.Image barcode = Barcode.encode(borrow.getBorrowCode());
            java.awt.Image qrcode = QRCode.encode(borrow.getBorrowCode());
            ...
            if (Desktop.isDesktopSupported()) {
                try {
                    File myFile = new File(Configuration.getInstance().getWorkspace() + "\\PDF_NOTITFICATION_" + borrow.getBorrowCode() + ".pdf");
                    Desktop.getDesktop().open(myFile);
                } catch (IOException ex) {
                    // no application registered for PDFs
                }
